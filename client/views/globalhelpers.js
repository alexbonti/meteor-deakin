...
Template.registerHelper('popupData', () => popupData.get());
...
Template.registerHelper('isEmpty', function (item) {
    var itemSize = Blaze._globalHelpers.sizeOf(item);
    if (itemSize == 0) {
        return true;
    } else {
        return (itemSize == 0);
...
Template.registerHelper('formatDateDefault', function (date, invalidDefault) {
    var format = 'ddd, D MMM YYYY, HH:mm';
    Blaze._globalHelpers.formatDate(date, format, invalidDefault);
});
